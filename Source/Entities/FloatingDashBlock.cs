...
public class FloatingDashBlock : Solid
{
    ...
    public FloatingDashBlock(EntityData data, Vector2 offset) : base(data.Position + offset, data.Width, data.Height, false)
    {
    ...
    private void Break(Vector2 from, Vector2 _, bool playSound = true, bool playDebrisSound = true)
    {
        if (playSound)
        {
            switch (tileType)
    ...
    private void AddJumpThru(JumpThru jp)
    {
        ...
        foreach (FloatingDashBlock entity in Scene.Tracker.GetEntities<FloatingDashBlock>().Cast<FloatingDashBlock>())
        {
